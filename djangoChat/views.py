...
def signup(request):
    ...
    if request.method=="POST":
        ...
        if userform.is_valid():
            ...
            return render(request, "djangoChat/signup.html", 
                          {"userform":userform,})
    elif request.method=="GET":
        userform=UserCreationForm()

        return render(request, "djangoChat/signup.html", {"userform":userform,})

def logout(request):
	cu = request.user.profile
	cu.is_chat_user = False
	cu.save()
	return redirect('djangoChat.views.login')
